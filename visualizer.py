...
algorithm_names = [[ "Bubble", "Merge", "Quick"],
                   ["Heap", "Insertion"]]
...
def draw(size, pos):
    ...
    for i, row in enumerate(range(rows)):
        for j, col in enumerate(range(cols)):
             if i < len(algorithm_names) and j < len(algorithm_names[i]):
                 x = col * (width/cols)
                 y = row * (height/rows)
...
def sleep_time(algorithm):
    if algorithm in ["Bubble Sort"]:
        return 0.01
    if algorithm in ["Heap Sort", "Merge Sort", "Quick Sort"]:
        return 0.005
    if algorithm in ["Insertion Sort"]:
        return 0.05
...
def update_screen(algorithm, swap_index_1=None, swap_index_2=None, pivot=None, screen=screen):
    ...
    for i in range(4*length):
        height_factor=algorithm.array[i//4]/2
        if i%4 != 0:
...
def main(screen):
    algorithm_name = None  # Initialize here
